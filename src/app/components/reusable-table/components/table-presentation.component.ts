...
@Component({
  ...
  template: `
    <div class="table-container" [class]="config?.cssClass || ''">
      ...
      <div class="pagination-top bg-surface border-b border-outline/12 px-4 py-2 flex items-center justify-between">
        <div class="pagination-info flex items-center gap-4">
          <span class="text-sm text-on-surface-variant" *ngIf="config?.pagination?.showPageInfo">
            {{ paginationInfo() }}
          </span>
        ...
        <div class="page-size-selector flex items-center gap-2">
          <span class="text-sm text-on-surface-variant">Itens por página:</span>
          <select 
      ...
      <!-- Tabela -->
      <div class="table-wrapper" [style.height]="config?.fixedHeight || 'auto'" [style.overflow-y]="config?.fixedHeight ? 'auto' : 'visible'">
        <table 
          mat-table 
          [dataSource]="data || []"
          matSort
          ...
          <ng-container matColumnDef="rowNumber" *ngIf="config?.showRowNumbers">
            ...
            <td mat-cell *matCellDef="let row; let i = index" class="w-16 text-center text-on-surface-variant">
              {{ getRowNumber(i) }}
            </td>
          </ng-container>
          
          <!-- Colunas dinâmicas -->
          <ng-container *ngFor="let column of visibleColumns(); trackBy: trackByColumn" [matColumnDef]="getColumnKey(column)">
            <th 
              mat-header-cell 
              *matHeaderCellDef 
              ...
              [attr.aria-label]="column.sortable ? 'Ordenar por ' + column.title : column.title">
              {{ column.title }}
            </th>
            <td 
              mat-cell 
          ...
          <tr 
            ...
            *matRowDef="let row; columns: displayedColumns(); let i = index"
            class="hover:bg-surface-variant/8 cursor-pointer transition-colors"
        ...
        <div 
          *ngIf="config?.loading" 
          ...
          <div class="flex flex-col items-center gap-2">
            ...
            <span class="text-sm text-on-surface-variant">Carregando...</span>
          </div>
        </div>
        ...
        <div 
          *ngIf="!config?.loading && (!data || data.length === 0)" 
          ...
          <mat-icon class="text-6xl text-on-surface-variant/50 mb-4">inbox</mat-icon>
          <p class="text-lg text-on-surface-variant">{{ config?.noDataMessage || 'Nenhum dado encontrado' }}</p>
        </div>
      </div>
    </div>
  `,
  styles: [`
    .table-container {
      @apply relative bg-surface rounded-lg border border-outline/12 overflow-hidden;
    }
    ...
    .mat-mdc-header-cell {
      @apply text-on-surface font-medium border-b border-outline/12;
    }
    
    .mat-mdc-cell {
      @apply text-on-surface border-b border-outline/8;
    }
    ...
    .mat-mdc-paginator {
      @apply bg-transparent border-t border-outline/12;
    }
  `]
})
export class TablePresentationComponent<T = any> implements OnInit {
  ...
  displayedColumns = computed(() => {
    ...
    const dataColumns = this.visibleColumns().map(col => this.getColumnKey(col));
  ...
  getRowNumber(index: number): number {
    ...
    return this.config.pagination.pageIndex * this.config.pagination.pageSize + index + 1;
  }
  ...
  formatCellValue(value: any, column: TableColumn<T>): string {
    ...
    switch (column.type) {
      case 'date':
        return value instanceof Date ? value.toLocaleDateString('pt-BR') : new Date(value).toLocaleDateString('pt-BR');
      case 'currency':
