...
@Component({
  selector: 'app-custom-mat-select4',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    MatSelectModule,
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => CustomMatSelect4Component),
      multi: true,
    },
  ],
  ...
  template: `
    <div class="custom-select-container">
      <mat-form-field [appearance]="appearance()" class="w-full">
        ...
        >
          <!-- Campo de busca nativo implementado -->
              ...
              <mat-form-field appearance="fill" class="search-field">
                ...
                @if (searchControl.value) {
                  <button
                    ...
                    type="button"
                  >
                    <mat-icon class="custom-clear-icon">{{
                      customClearIcon()
                    }}</mat-icon>
                  </button>
                }
                ...
                @if (isSearching()) {
                  <mat-spinner matSuffix diameter="20" class="search-spinner" />
                }
              </mat-form-field>
            </div>
          </mat-option>
          ...
          @if (showPaginationInfo() && allOptions().length > 0) {
            <mat-option disabled class="pagination-info-option">
          ...
          @if (!hasMoreItems() && allOptions().length > 0 && !isSearching()) {
            <mat-option disabled class="info-option">
          ...
          @if (
            filteredOptions().length === 0 && !isSearching() && !isLoadingMore()
          ) {
            <mat-option disabled class="info-option">
              <div class="info-content">
  ...
  styles: [
    `
      ...
      /* Evitar que o select feche ao clicar no campo de busca */
      ::ng-deep .search-option:hover {
        background-color: #f8f9fa !important;
      }

      .dark ::ng-deep .search-option:hover {
        background-color: #2d2d2d !important;
      }
    `,
  ],
})
export class CustomMatSelect4Component
  implements OnInit, OnDestroy, AfterViewInit, ControlValueAccessor
{
  ...
  readonly paginationInfoTemplate = input<string>(
  ...
  readonly searchFunction =
    input.required<(params: SearchParams) => Observable<ApiResponse>>();
  ...
  private loadInitialData() {
    ...
    if (!searchFunction) {
      console.warn(
        'searchFunction não foi fornecida para CustomMatSelect4Component',
      );
      return;
    }
